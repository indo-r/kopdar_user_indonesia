...
```{r}
remotes::install_github("aswansyahputra/nusandata") # memasang paket
library(nusandata) # mengaktifkan paket
```
...
**Jawaban:** simbol `::` digunakan untuk menggunakan fungsi dari suatu paket tanpa harus mengaktifkan paket tersebut melalui `library(nama_paket)` terlebih dahulu. Struktur penulisannya adalah `nama_paket::nama_fungsi`. Simbol `::` digunakan untuk menggunakan *exported function*, yaitu fungsi yang secara baku ditayangkan kepada pengguna saat paket diaktifkan. Sedangkan simbol `:::` digunakan untuk menggunakan *non-exported function*, yaitu fungsi yang secara baku tidak ditayangkan pengguna saat paket diaktifkan (fungsi internal dalam paket).
...
head(stopwords_id, 20) # melihat 20 kata pertama
```
...
df2 <- separate_rows(df1, teks, sep = " ")
...
### Kode Preparasi Dataset Format Pipeline

**Tugas:** Membuat kode preparasi dataset yang tersedia kedalam bentuk *pipeline* dengan menggunakan `%>%`. Ingat data `tks_debatcapres1_2019` harus ditulis pada baris pertama dan hasil akhir disimpan dalam obyek bernama `df`! 
...
### Alur Preparasi Dataset

**Tugas:** Membuat ringkasan alur yang ditempuh dengan memahami fungsi yang dijalankan pada setiap langkah hingga dataset siap untuk dibuat menjadi grafik. Contoh: 1. Membuat teks kedalam huruf kecil dan menghilangkan..., dan seterusnya. Sertakan perubahan yang terjadi pada dataset pada setiap langkah. Anda dapat membaca tujuan dan dokumentasi suatu fungsi dengan menjalankan `?namafungsi` pada konsol.

**Jawaban:** 1) Membuat karakter dalam kolom teks kedalam format furuf kecil dan menghilangkan tanda baca; 2) Memisahkan karakter dalam kolom teks sehingga menjadi kata-per-kata dengan menggunakan spasi sebagai penanda antar kata. Setiap kata disimpan dalam satu baris; 3) Membuang baris kata apabila nilainya adalah spasi dan termasuk dalam *stopwords*; 4) Menghitung frekuensi penggunaan kata berdasarkan penutur dan mengurutkan hasilnya berdasarkan nilai tertinggi; 5) Membuat pengelompokan data berdasarkan penutur; 6) Mengambil 25 kata dengan frekuensi penggunaan tertinggi dari setiap penutur. Catatan: Hasil yang didapatkan dapat lebih dari 25 kata apabila terdapat frekuensi yang sama; 7) Menghapus pengelompokan data berdasarkan penutur; 8) Membuang baris apabila frekuensi penggunaan kata kurang dari 3 kali.

### Grafik Frekuensi Kata

**Tugas:** Memperbaiki grafik frekuensi kata dengan ketentuan:
...
+ [*For experienced user:*] Modifikasi dataset `tks_debatcapres1_2019` sehingga penutur dikategorikan menjadi 3, yaitu "Paslon 01", "Paslon 02", "Moderator". Hilangkan penutur "Arief Budiman" dan "Pembicara" sebelum membuat kategori! Anda dapat menggunakan pendekatan *nested ifelse*, `%in%`, `case_between`, atau lainnya.
...
df %>% 
  ...
  coord_flip() +
  labs(
    title = "Frekuensi Penggunaan Kata oleh Joko Widodo",
    subtitle = "Debat Capres 1 pada 17 Januari 2019",
...
# Tambahan: membuat tema sebagai fungsi untuk digunakan pada grafik berikutnya
...
df %>% 
  ...
  coord_flip() +
  labs(
    title = "Frekuensi Penggunaan Kata oleh Prabowo Subianto",
    subtitle = "Debat Capres 1 pada 17 Januari 2019",
    caption = "Sumber: debatcapres.bahasakita.co.id",
    x = "",
    y = "Frekuensi Kata"
...
```{r}
df %>% 
  filter(penutur == "Maruf Amin") %>% 
  ggplot(aes(x = fct_reorder(teks, n), y = n)) +
  geom_col(fill = "grey90", col = "grey60") +
  coord_flip() +
  labs(
    title = "Frekuensi Penggunaan Kata oleh Maruf Amin",
    subtitle = "Debat Capres 1 pada 17 Januari 2019",
    caption = "Sumber: debatcapres.bahasakita.co.id",
    x = "",
    y = "Frekuensi Kata"
...
```{r}
df %>% 
  filter(penutur == "Sandiaga Uno") %>% 
  ggplot(aes(x = fct_reorder(teks, n), y = n)) +
  geom_col(fill = "grey90", col = "grey60") +
  coord_flip() +
  labs(
    title = "Frekuensi Penggunaan Kata oleh Sandiaga Uno",
    subtitle = "Debat Capres 1 pada 17 Januari 2019",
    caption = "Sumber: debatcapres.bahasakita.co.id",
    x = "",
    y = "Frekuensi Kata"
...
plot_kata <- function(data, penutur) {
  ...
  penutur_quo <- enquo(penutur)

    data %>% 
    filter(penutur == !!penutur_quo) %>% 
    ggplot(aes(x = fct_reorder(teks, n), y = n)) +
    geom_col(fill = "grey90", col = "grey60") +
    coord_flip() +
    labs(
...
```{r}
plot_kata(data = df, penutur = "Ira Koesno")
plot_kata(data = df, penutur = "Imam Priyono")
```
...
```{r}
plot_kata(data = df_sederhana, penutur = "Paslon 01")
```

```{r}
plot_kata(data = df_sederhana, penutur = "Paslon 02")
```

```{r}
df_sederhana %>% 
  filter(penutur %in% c("Paslon 01", "Paslon 02")) %>% 
  ggplot(aes(x = fct_reorder(teks, n), y = n)) +
  geom_col(fill = "grey90", col = "grey60") +
  coord_flip() +
  facet_wrap(~penutur, scales = "free") +
  labs(
    title = "Frekuensi Penggunaan Kata oleh Kedua Paslon",
    subtitle = "Debat Capres 1 pada 17 Januari 2019",
    caption = "Sumber: debatcapres.bahasakita.co.id",
    x = "",
    y = "Frekuensi Kata"
